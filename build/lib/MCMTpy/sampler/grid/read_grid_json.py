...
def read_Inv_json(filename):
    ...
    for d in range(0,depth_num,1):
        ...
        if depth > FM_boundary[3][1]:
            depth = FM_boundary[3][1]
        ...
        for i in range(0,strike_num,1):
            ...
            if strike > FM_boundary[0][1]:
                strike = FM_boundary[0][1]
            ...
            for j in range(0,dip_num,1):
                ...
                if dip > FM_boundary[1][1]:
                    dip = FM_boundary[1][1]

                for k in range(0,rake_num,1):
                    if Inversion_json["Fixed_FM"][3]=='constant':
                        rake = Rake
                    else:
                        rake = FM_boundary[2][0] + k*Inversion_json["step_rake"]
                    if rake > FM_boundary[2][1]:
                        rake = FM_boundary[2][1]
