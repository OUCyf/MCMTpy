...
#%% subfunction
############################################
def write_inv_para(file_path,Inv_para,Sta_inv_info,Sta_data,Stf):
...
############################################
def  get_sigma(Inv_para,jj):
    #  3 (sf), 4(dc) or 7(mt)
    InvType=Inv_para["InvType"]

    sigma_mw = Inv_para["sigma_mw"]
    sigma_mt = Inv_para["sigma_mt"]
    sigma_dc = Inv_para["sigma_dc"]
    sigma_sf = Inv_para["sigma_sf"]
    sigma_lat_lon = Inv_para["sigma_lat_lon"]
    sigma_depth = Inv_para["sigma_depth"]
    sigma_t = Inv_para["sigma_t"]

    if InvType=='mt':
        sigma=[sigma_mw, sigma_mt, sigma_mt, sigma_mt, sigma_mt, sigma_mt, sigma_mt,
          sigma_lat_lon, sigma_lat_lon, sigma_depth, sigma_t ]
    elif InvType=='dc':
        sigma=[sigma_mw, sigma_dc, sigma_dc, sigma_dc,
          sigma_lat_lon, sigma_lat_lon, sigma_depth, sigma_t ]
    elif InvType=='sf':
        sigma=[sigma_mw, sigma_sf, sigma_sf, 
           sigma_lat_lon, sigma_lat_lon, sigma_depth, sigma_t ]

    return sigma


def get_fixed_fm(Inv_para,jj):
    #  3 (sf), 4(dc) or 7(mt)
    InvType=Inv_para["InvType"]
    ...
    if jj<N_k:
        if InvType=='mt':
             const = ['constant',
                      ...
                      'constant','constant','constant']
             Fixed_FM_used = const+Fixed_FM[7:11]
        elif InvType=='dc':
             const = ['constant',
                      'constant','constant','constant']
             Fixed_FM_used = const+Fixed_FM[4:8]
        elif InvType=='sf':
             const = ['constant',
                      'constant','constant']
             Fixed_FM_used = const+Fixed_FM[3:7]

    else:
        if InvType=='mt':
             Fixed_FM_used = Fixed_FM[0:11]
        elif InvType=='dc':
             Fixed_FM_used = Fixed_FM[0:8]
        elif InvType=='sf':
             Fixed_FM_used = Fixed_FM[0:7]

    return Fixed_FM_used
...
# ############################################
# def truncate_time_series(np1,np2,shift):  # not used
...
def get_MISFIT_2(GFs_sta_info,Sta_raw_data,Sta_rand_data,Sta_inv_info,Inv_para,Prepro_para,T0):
    dt = Inv_para["Dt"]
        ...
        if Inv_para['Geometrical_spreading_mode']==True:
            ...
            for i in range (0,3,1):                                             # p wave in ZRT comp
                if P_inv_comp[i]=='yes':
                    ...
                    MISFIT1_one_sta += 1/P_win_length * Phase_weight[0] * (distance/Distance_0)**2*math.sqrt(np.mean((Raw_data_np_p-Rand_data_np_p_shift)**2)) # * (1-C_max_maxlag)
            ...
            for i in range (0,3,1):                                             # S wave in ZRT comp
                if S_inv_comp[i]=='yes':
                    ...
                    MISFIT1_one_sta += 1/S_win_length * Phase_weight[1] * (distance/Distance_0)**2*math.sqrt(np.mean((Raw_data_np_s-Rand_data_np_s_shift)**2)) #  * (1-C_max_maxlag)
            ...
            for i in range (0,3,1):                                             # Surf wave in ZRT comp
                if Surf_inv_comp[i]=='yes':
                    ...
                    MISFIT1_one_sta += 1/Surf_win_length * Phase_weight[2] * (distance/Distance_0)**1*math.sqrt(np.mean((Raw_data_np_surf-Rand_data_np_surf_shift)**2)) #  * (1-C_max_maxlag)
            ...
            for i in range (0,3,1):                                             # p wave in ZRT comp
                if P_inv_comp[i]=='yes':
                    ...
                    MISFIT1_one_sta += 1/P_win_length * Phase_weight[0]*math.sqrt(np.mean((Raw_data_np_p-Rand_data_np_p_shift)**2)) # *(1-C_max_maxlag)
                    M0_one_sta.append( np.linalg.norm(Raw_data_np_p) / np.linalg.norm(Rand_data_np_p_shift) )
            ...
            for i in range (0,3,1):                                             # S  wave in ZRT comp
                if S_inv_comp[i]=='yes':
                    ...
                    MISFIT1_one_sta += 1/S_win_length * Phase_weight[1]*math.sqrt(np.mean((Raw_data_np_s-Rand_data_np_s_shift)**2)) # *(1-C_max_maxlag)
                    M0_one_sta.append( np.linalg.norm(Raw_data_np_s) / np.linalg.norm(Rand_data_np_s_shift) )
            ...
            for i in range (0,3,1):                                             # Surf  wave in ZRT comp
                if Surf_inv_comp[i]=='yes':
                    ...
                    MISFIT1_one_sta += 1/Surf_win_length * Phase_weight[2]*math.sqrt(np.mean((Raw_data_np_surf-Rand_data_np_surf_shift)**2)) # *(1-C_max_maxlag)
                    M0_one_sta.append( np.linalg.norm(Raw_data_np_surf) / np.linalg.norm(Rand_data_np_surf_shift) )
    ...
    return MISFIT1/Inv_para["Station_num"], Mw_mean




############################################
def get_MISFIT_1(FM_new,Sta_raw_data,Sta_rand_data):
    '''
    Note:
    ...
    '''
    MISFIT2 = 0
    net_sta_name = list(Sta_rand_data.keys())
    N_sta = len(Sta_rand_data)
    
    for m in range(0,N_sta,1):
        Raw_tp = Sta_raw_data[net_sta_name[m]]['tp']
        Rand_tp = Sta_rand_data[net_sta_name[m]]['tp']
        Raw_ts = Sta_raw_data[net_sta_name[m]]['ts']
        Rand_ts = Sta_rand_data[net_sta_name[m]]['ts']
        tp_misfit = (Raw_tp-FM_new[-1]-Rand_tp)**2
        ts_misfit = (Raw_ts-FM_new[-1]-Rand_ts)**2
        
        MISFIT2 = MISFIT2+tp_misfit+ts_misfit

    return MISFIT2/N_sta




############################################
def inv_output_file(filepath,chain,FM_1_all,FM_1_accept_all,FM_2_all,FM_2_accept_all,MISFIT_1_all,MISFIT_2_all,
                    MISFIT_1_accept_all,MISFIT_2_accept_all,accpet_ratio_all):
    if not os.path.isdir(filepath):os.mkdir(filepath)
    
    ## FM_1_all
    file=os.path.join(filepath,'chain_'+str(chain)+'_'+'FM_1_all')
    fp = open(file, 'w')
    for  i in  range(0,len(FM_1_all),1):
        for  j in  range(0,len(FM_1_all[0]),1):
            fp.write('{:.8f}'.format(FM_1_all[i][j]))
            fp.write('   ')
        fp.write('\n')
    fp.close()
    
    ## FM_2_all
    file=os.path.join(filepath,'chain_'+str(chain)+'_'+'FM_2_all')
    fp = open(file, 'w')
    for  i in  range(0,len(FM_2_all),1):
        for  j in  range(0,len(FM_2_all[0]),1):
            fp.write('{:.8f}'.format(FM_2_all[i][j]))
            fp.write('   ')
        fp.write('\n')
    fp.close()
    
    ## FM_1_accept_all
    file=os.path.join(filepath,'chain_'+str(chain)+'_'+'FM_1_accept_all')
    fp = open(file, 'w')
    for  i in  range(0,len(FM_1_accept_all),1):
        for  j in  range(0,len(FM_1_accept_all[0]),1):
            fp.write('{:.8f}'.format(FM_1_accept_all[i][j]))
            fp.write('   ')
        fp.write('\n')
    fp.close()
    
    ## FM_2_accept_all
    file=os.path.join(filepath,'chain_'+str(chain)+'_'+'FM_2_accept_all')
    fp = open(file, 'w')
    for  i in  range(0,len(FM_2_accept_all),1):
        for  j in  range(0,len(FM_2_accept_all[0]),1):
            fp.write('{:.8f}'.format(FM_2_accept_all[i][j]))
            fp.write('   ')
        fp.write('\n')
    fp.close()

    ## MISFIT_1_all
    file=os.path.join(filepath,'chain_'+str(chain)+'_'+'MISFIT_1_all')
    fp = open(file, 'w')
    for  i in  range(0,len(MISFIT_1_all),1):
        fp.write('{:.8f}'.format(MISFIT_1_all[i]))
        fp.write('   ')
        fp.write('\n')
    fp.close()
    ...
    ## MISFIT_2_all
    file=os.path.join(filepath,'chain_'+str(chain)+'_'+'MISFIT_2_all')
    fp = open(file, 'w')
    for  i in  range(0,len(MISFIT_2_all),1):
        fp.write('{:.8f}'.format(MISFIT_2_all[i]))
        fp.write('   ')
        fp.write('\n')
    fp.close()

    ## MISFIT_1_accept_all
    file=os.path.join(filepath,'chain_'+str(chain)+'_'+'MISFIT_1_accept_all')
    fp = open(file, 'w')
    for  i in  range(0,len(MISFIT_1_accept_all),1):
        fp.write('{:.8f}'.format(MISFIT_1_accept_all[i]))
        fp.write('   ')
        fp.write('\n')
    fp.close()

    ## MISFIT_2_accept_all
    file=os.path.join(filepath,'chain_'+str(chain)+'_'+'MISFIT_2_accept_all')
    fp = open(file, 'w')
    for  i in  range(0,len(MISFIT_2_accept_all),1):
        fp.write('{:.8f}'.format(MISFIT_2_accept_all[i]))
        fp.write('   ')
        fp.write('\n')
    fp.close()

    ## accpet_ratio
    file=os.path.join(filepath,'chain_'+str(chain)+'_'+'accpet_ratio_all')
    fp = open(file, 'w')
    for  i in  range(0,len(accpet_ratio_all),1):
        fp.write('{:.8f}'.format(accpet_ratio_all[i]))
        fp.write('   ')
        fp.write('\n')
    fp.close()
